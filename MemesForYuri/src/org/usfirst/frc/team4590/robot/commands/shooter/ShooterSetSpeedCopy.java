...
public class ShooterSetSpeedCopy extends Command implements PIDSource, PIDOutput {
	...
	private double m_power = RPM1925_POWER;
	
	private static final double RPM1925_POWER = -0.782;//-0.777;//-0.64; // -0.5663;// -0.6325;
	...
	protected void execute() {
		...
		if (Feeder.getInstance().getCurrentCommand() != null && m_controller.getI() != 0 && Feeder.getInstance().getCurrentCommand() instanceof FeedToShooter){
			m_power = m_lastPower;
	...
	public void pidWrite(double output) {
		...
		SmartDashboard.putNumber("Power value to motors", pdp_multi * (m_power - output));
		Shooter.getInstance().setPower(pdp_multi * (m_lastPower = m_power - output));
	}
