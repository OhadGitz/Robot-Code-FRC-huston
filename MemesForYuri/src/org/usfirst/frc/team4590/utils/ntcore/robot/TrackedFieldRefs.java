...
public class TrackedFieldRefs<T> implements ITableListener{
	...
	protected boolean syncField(String field, Object value){
		System.out.println("Syncing field '" + field + "' with value " + value);
	...
	@Override
	public void valueChanged(ITable source, String key, Object value, boolean isNew) {
		System.out.println("valueChanged(" + source + "," + key + "," + value + "," + isNew + ")");
		if (fieldExists(m_path + NetworkTable.PATH_SEPARATOR + key)){
		...
		for (Entry<String, FieldRef<T>> entry : m_fields.entrySet()){
			...
			if (path.contains("" + NetworkTable.PATH_SEPARATOR)){
				...
				if (!m_slaves.contains(tableName)){
					ITable table = NetworkTable.getTable("data_sync/robot").getSubTable(m_path).getSubTable(tableName);
